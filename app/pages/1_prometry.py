...
if st.button("Calulate dataframe"):
    ...
    code_string += "\ngm = pg.GeometryMaker(pobjs)\n"
    code_string += "df = gm.calculateGeometry(ls_geos)\n"
...
if len(df.index) > 0:
    st.dataframe(df)
    st.download_button("Download promerty file",df.to_csv(),"promerty.csv","text/csv",key='download-csv')

    with st.expander("Expand for code to replicate dataframe generation"):
    ...
    if st.button("Calulate plot"):
        cols = st.columns(2)
        with cols[0]:
            fig = px.scatter(df, x=x_ax1, y=y_ax1, color=z_ax1,title="",width=500, height=500, opacity=0.7,color_continuous_scale=px.colors.sequential.Viridis)
            st.plotly_chart(fig, use_container_width=False)
        with cols[1]:
            fig = px.scatter(df, x=x_ax2, y=y_ax2, color=z_ax2,title="",width=500, height=500, opacity=0.7,color_continuous_scale=px.colors.sequential.Viridis)
            st.plotly_chart(fig, use_container_width=False)

        code_string = "import plotly.express as px\n"
        code_string += f"fig = px.scatter(df, x='{x_ax1}', y='{y_ax1}', color='{z_ax1}',title="",width=500, height=500, opacity=0.7, color_continuous_scale=px.colors.sequential.Viridis))\n"
        code_string += "fig.show() #or preferred method, e.g. fig.write_html('path/to/file.html')"
                    
        with st.expander("Expand for code to replicate plot"):
            st.code(code_string)
